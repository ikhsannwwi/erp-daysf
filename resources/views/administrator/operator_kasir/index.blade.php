...
@section('content')
                    ...
                    <div class="col-6">
                        ...
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active" aria-current="page">Operator Kasir</li>
                            </ol>
                        </nav>
                    </div>
                    <div class="col-6">
                        @if (isallowed('operator_kasir', 'add'))
                            <a href="{{ route('admin.operator_kasir.add') }}" class="btn btn-primary me-3 float-end">Tambah
                                Data</a>
                        @endif
                        @if (isallowed('operator_kasir', 'arsip'))
                            <a href="{{ route('admin.operator_kasir.arsip') }}" class="btn btn-primary mx-3 float-end">Arsip</a>
                        @endif
                        <a href="javascript:void(0)" class="btn btn-primary float-end" id="filterButton">Filter</a>
                    </div>
                </div>
            </div>
            @include('administrator.operator_kasir.filter.main')
            <div class="card-body">
    ...
    @include('administrator.operator_kasir.modal.detail')
@endsection
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            var data_table = $('#datatable').DataTable({
                ...
                ajax: {
                    url: '{{ route('admin.operator_kasir.getData') }}',
                    dataType: "JSON",
                    type: "GET",
                    data: function(d) {
                        d.status = getStatus();
            ...
            $(document).on('click', '.delete', function(event) {
                ...
                swalWithBootstrapButtons.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            type: "DELETE",
                            url: "{{ route('admin.operator_kasir.delete') }}",
                            data: {
                                "_token": "{{ csrf_token() }}",
                                "_method": "DELETE",
                                "id": id,
            ...
            $(document).on('click', '.changeStatus', function(event) {
                ...
                swalWithBootstrapButtons.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            type: "POST",
                            url: "{{ route('admin.operator_kasir.changeStatus') }}",
                            data: ({
                                "_token": "{{ csrf_token() }}",
                                "_method": "POST",
                                ix: ix,
            ...
            $('#filter_submit').on('click', function(event) {
                ...
                data_table.ajax.url('{{ route('admin.operator_kasir.getData') }}?status=' + filterStatus +
