<!-- Modal Detail Gudang -->
<div class="modal fade" id="detailGudang" tabindex="-1" aria-labelledby="detailGudangLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="detailGudangLabel">Detail Gudang</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body" id="detailGudangBody">
...
@push('js')
    <script>
        $('#detailGudang').on('show.bs.modal', function(event) {
            var button = $(event.relatedTarget);
            var id = button.data('id');

            var modalBody = $('#detailGudangBody');
            ...
            $.ajax({
                url: '{{ route('admin.gudang.getDetail', ':id') }}'.replace(':id', id),
                method: 'GET',
