    ...
    <section class="section">
        <div class="card">
            ...
            <div class="card-content">
                <div class="card-body">
                    <form action="{{ route('admin.produk_promo.update') }}" method="post" enctype="multipart/form-data"
                            ...
                            <div class="col-md-6 col-12">
                                <div class="form-group mandatory">
                                    ...
                                    <input type="text" id="inputNama" class="form-control" placeholder="Masukan Nama"
                                        name="nama" autocomplete="off" data-parsley-required="true" value="{{$data->nama}}">
                            ...
                            <div class="col-md-6 col-12">
                                <div class="form-group mandatory">
                                    ...
                                    <select class="wide mb-2" name="jenis" id="inputJenis"
                                        ...
                                        <option value="persentase" {{$data->jenis === 'persentase' ? 'selected' : ''}}>Persentase</option>
                                        <option value="harga_tetap" {{$data->jenis === 'harga_tetap' ? 'selected' : ''}}>Harga Tetap</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                                            ...
                                            <tbody>
                                                ...
                                                <tr class="detail-list" childidx="{{$key}}" style="position: relative;">
                                                    ...
                                                    <td class="text-end text-sm">
                                                        ...
                                                        <input type="hidden" name="detail[{{$key}}][diskon_harga_tetap]" class="form-control text-end diskon_harga_tetap-item" value="{{$data->jenis === 'harga_tetap' ? 'Rp ' . number_format($row->diskon, 0, ',', '.') : ''}}"
                                                            data-parsley-required="false" autocomplete="off">
                                                        <span class="setelah_konversi_persentase-item">{{$data->jenis === 'persentase' ? 'Rp ' . number_format($row->diskon, 0, ',', '.') : ''}}</span>
                                                        <input type="hidden" name="detail[{{$key}}][diskon_persentase_harga]" class="diskon_persentase_harga-item" value="{{$data->jenis === 'persentase' ? $row->diskon : ''}}">
                                                    </td>
                                                    <td class="text-end">
                                                        <span class="stok_tersedia-item"></span>
                                                    </td>
                                                    <td class="text-end text-sm">
                                                        <input type="text" name="detail[{{$key}}][total_stok_promo]" class="form-control text-end total_stok_promo-item" value="{{$row->total_stok_promo === 'Tidak Terbatas' ? 'Tidak Terbatas' : number_format($row->total_stok_promo, 0, ',', '.')}}"
                                                            data-parsley-required="true" autocomplete="off" {{$row->total_stok_promo === 'Tidak Terbatas' ? 'readonly' : ''}}>
                                                            <div class="form-check">
                                                                <label class="form-check-label form-label">
                                                                    Tetapkan Batas
                                                                </label>
                                                                <input class="form-check-input checkbox_total_stok_promo-item" type="checkbox" name="detail[{{$key}}][checkbox_total_stok_promo]" {{$row->total_stok_promo === 'Tidak Terbatas' ? '' : 'checked'}}
                                                                    value="1">
                                                            </div>
                                                    <td class="text-end text-sm">
                                                        <input type="text" name="detail[{{$key}}][batas_pembelian]" class="form-control text-end batas_pembelian-item" value="{{$row->batas_pembelian === 'Tidak Terbatas' ? 'Tidak Terbatas' : number_format($row->batas_pembelian, 0, ',', '.')}}"
                                                        data-parsley-required="true" autocomplete="off" {{$row->batas_pembelian === 'Tidak Terbatas' ? 'readonly' : ''}}>
                                                        <div class="form-check">
                                                            <label class="form-check-label form-label">
                                                                Tetapkan Batas
                                                            </label>
                                                            <input class="form-check-input checkbox_batas_pembelian-item" type="checkbox" name="detail[{{$key}}][checkbox_batas_pembelian]" {{$row->batas_pembelian === 'Tidak Terbatas' ? '' : 'checked'}}
                                                                value="1">
                                                        </div>
                                                    </td>
                                                    <td class="text-center"><a href="javascript:void(0)" class="btn btn-outline-danger removeData"
                                                        style="display: {{count($data->detail) > 1 ? 'block' : 'none'}}" data-ix="{{ $row->id }}"
                                                        data-promo_produk_id="{{ $row->promo_produk_id }}"><i
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('#daftar_detail').on('click', '.removeData', function() {
                let another = this;
                let ix = $(this).data('ix');
                let produk_promo_id = $(this).data('produk_promo_id');
                ...
                swalWithBootstrapButtons.fire({
                    ...
                    if (result.isConfirmed) {
                        if (ix != '') {
                            $.ajax({
                                type: "DELETE",
                                url: "{{ route('admin.produk_promo.deleteDetail') }}",
                                data: {
                                    "_token": "{{ csrf_token() }}",
                                    "_method": "DELETE",
                                    "id": ix,
            ...
            function resetData() {
                ...
                $(".detail-list").each(function() {
                    ...
                    if ($('#inputJenis').val() === 'persentase') {
                        ...
                        $(this).find('.setelah_konversi_persentase-item').text('');
